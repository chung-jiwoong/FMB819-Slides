...
3.  1–2를 49번 반복 → 원래 표본 크기($n=50$)와 동일한 새 표본
4.  새 표본에서 $\hat{p}$ 계산
...
> ❌ **틀린 해석**: "모집단 모수가 이 구간 안에 있을 확률이 95%다."
>
> ✅ **정확한 해석**: 동일한 방법으로 신뢰 구간을 반복 생성하면, 그 중 **약 95%가 모집단 모수를 포함**할 것으로 기대된다.
...
-   그런데 경영 현장의 질문은 종종 이런 형태:
    ...
    -   A안과 B안 중 어느 쪽이 **진짜 더 효과적**인가? (A/B 테스트)
...
> 성차별이 **없는** 세계에서, "무작위로 섞었을 때 29.2%p 이상의 차이가 나올 확률"이 얼마나 되는가?
...
visualize(null_distribution, bins = 10, fill = "darkred") + 
  ...
  labs(title = "귀무분포: 성차별이 없는 세계에서의 승진율 차이",
...
-   **검은 영역** = p-값: $H_0$ 하에서 0.292 이상의 차이가 나올 확률.
