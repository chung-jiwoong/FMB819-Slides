...
-   ⚠️ **핵심 문제**: 개별 처치 효과 $\delta_i$는 **개인 수준에서는 절대 관측할 수 없음!** 대안: **집단 평균(population averages)**을 이용해 추정할 수 있음.
...
-   데이터 다운로드 [링크](https://raw.githack.com/chung-jiwoong/FMB819-Slides/refs/heads/main/chapter_causality/data/toy_data_2.csv). `read.csv()`를 사용하여 데이터를 불러오기.
...
-   새로운 데이터 다운로드 [링크](https://raw.githack.com/chung-jiwoong/FMB819-Slides/refs/heads/main/chapter_causality/data/toy_data_random.csv).  이 데이터에서는 동일한 개인을 **임의로 무작위 배정(random assignment)**하였음.
...
-   이후 **공공정책 평가(public policy evaluation)의 신뢰할 수 있는 도구**로 점차 자리 잡음. 👉 [참고 자료](https://www.povertyactionlab.org/fr)
...
📂 1. **데이터 불러오기**: 데이터 다운로드 [링크](https://raw.githack.com/chung-jiwoong/FMB819-Slides/refs/heads/main/chapter_causality/data/star_data.csv)\
...
```{r, echo=FALSE}
star_df = read_csv("https://raw.githack.com/chung-jiwoong/FMB819-Slides/refs/heads/main/chapter_causality/data/star_data.csv")
```
...
✅ **소규모 학급(small class)에 배정된 것이 수학 성적(math score)에 미치는 ATE를 추정해보자.**
