...
subtitle: "<span style='font-size:1.5em; color:#a01616;'>Tidying, Visualising and Summarising Data</span>"
# date: "`r Sys.Date()`"
format:
  revealjs:
    slide-number: true
...
-   다음 두 강의에서는 **데이터 정리, 시각화, 요약**의 기본을 배울 것임.
...
-   두 방법 모두 장점과 단점이 있으며, 여기서는 `dplyr`을 먼저 다룸.

------------------------------------------------------------------------

## `dplyr` 개요

-   [Hadley Wickham의 데이터 변환 챕터](https://r4ds.had.co.nz/transform.html)를 읽어보길바람.
-   [치트시트](https://github.com/rstudio/cheatsheets/blob/master/data-transformation.pdf)도 매우 유용.
-   `dplyr`은 여러 **verbs** 중심으로 구성되어 있음.
-   `data.frame` 또는 `tibble`\[\^`data.frame`과 유사\] 형식의 데이터를 다룰 것임.
...
polls_us_election_2016 %>%
    ...
    filter(trump_clinton_diff>5 &
           state == "Iowa" &
           is.na(rawpoll_johnson)) %>%
    select(pollster) # pollster(여론조사 기관) 열만 선택하여 데이터프레임으로 반환
```
:::
...
#samplesize 값들의 평균을 계산하되, NA(결측값)는 제거 
```
:::
...
# 총 행 개수를 계산 (즉, 수행된 여론조사의 총 개수) 
```
:::
:::::

-   `%>%` 연산자는 magrittr 패키지에서 제공. R v4.1.0부터 기본 제공(native) 파이프 연산자인 `|>` 추가

```{r, echo=TRUE}
polls_us_election_2016$samplesize |> mean(na.rm = TRUE)
```

`|>` 연산자를 사용하면 `magrittr` 패키지를 설치하지 않아도 됨
...
4.  표본 크기가 2,000명 이상인 여론조사에서 트럼프의 평균 지지율이 가장 높은 주는? (힌트: `filter(), group_by(), summarise(), arrange()` 사용. 내림차순 정렬하려면 `arrange()` 함수의 도움말을 참고)
...
1.  대륙별 연도별 평균 인구(mean_pop)를 계산하고, 결과를 새로운 객체 gapminder_mean에 저장하시오. (힌트: 각 연도별 대륙당 하나의 관측치(행)만 있어야 함. `group_by` 및 `summarise` 사용)
...
- y축을 로그 스케일로 변환 <br>`scale_fill_discrete()`<br>`scale_color_manual()`
```
:::
:::::
...
1.  2015년 기대수명(Life Expectancy)의 히스토그램을 작성하시오. (*힌트: 히스토그램을 만들 때 aes()에 y 값을 지정해야 할까?*) `geom_*` 내에서 다음 옵션을 설정하시오: `binwidth` = 5, `boundary` = 45, `colour` = "white", `fill` = "#d90502". 이러한 옵션이 무엇을 의미하는지 설명하시오.
    ...
    *Optional:* 생성한 히스토그램을 대륙(Continent)별로 나누어 (`facet_grid()`) 각 대륙이 새로운 행(row)에 표시되도록 만드시오.

2.  연도별 대륙별 평균 기대수명(Boxplot)을 작성하시오. `geom_*` 옵션: `colour` = "black", `fill` = "#d90502". (*힌트: continent 및 year 두 개의 변수로 그룹화해야함*)

3.  2015년 출산율(Fertility Rate, y축)과 영아 사망률(Infant Mortality, x축) 간의 관계를 나타내는 산점도를 작성하시오. `geom_*` 옵션: `size` = 3, `alpha` = 0.5, `colour` = "#d90502". `labs()`를 사용하여 레이블을 추가

# Summarising

## Summarising Data

-   일반적으로 데이터를 시각화하거나 요약 통계를 통해 데이터를 분석할 수 있음.
...
여유 시간이 있을 때, 다음 튜토리얼을 실행해보시오:

````         
```r
library(ScPoApps) # 설치하는 데 시간이 걸릴 수 있음
