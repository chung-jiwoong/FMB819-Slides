...
-   지수 수준(level)을 일간 **로그 수익률(log return)**로 변환하고, 단위를 해석하기 쉽도록 **bp(basis points, 1bp = 0.01%)**로 표현함.
-   로그 수익률을 사용하는 이유: 로그 수익률은 지수 수준의 곱셈적 관계를 덧셈적 관계로 변환하여 분석을 단순화함. 또한, 로그 수익률은 연속적으로 복리되는 수익률을 나타내는 데 적합함.
...
1.  코스피200 및 S&P500 일별 종가 데이터를 불러와 `returns_raw` 객체를 생성하시오.\
    **힌트**: 날짜를 기준으로 두 지수를 병합한 뒤, `log`와 `lag`를 사용해 수익률을 계산하면 됨.

```{r}
sp500_px <- read_csv("https://stooq.com/q/d/l/?s=^spx")
head(sp500_px)
ks200_px <- read_csv("https://stooq.com/q/d/l/?s=^ks200")
head(ks200_px)
```

```{r}
sp500_px <- read_csv("https://raw.githubusercontent.com/gustavek/teaching/master/data/sp500_px.csv")
head(sp500_px)
```
...
2.  데이터셋 설명:
    ...
    -   총 몇 개의 관측치가 있는가?\
    -   데이터셋을 확인하고 어떤 변수가 있는지 확인하시오. `ret_sp500_bp`, `ret_kospi200_bp`, `ret_kospi200_lead_bp`는 각각 무엇을 의미하는가?\
    -   `skimr` 패키지의 `skim` 함수 사용해서 `ret_sp500_bp`, `ret_kospi200_bp`, `ret_kospi200_lead_bp` 변수에 대한 기본적인 요약 통계 확인함.\
        **힌트**: `dplyr`의 `select` 사용해서 변수 선택한 후 `%>%`로 `skim()` 적용하면 됨.

3.  코스피200 지수와 S&P500 지수 간 실제 (선형) 관계에 대해 어떤 관계가 있을 것이라 생각하는가?

4.  S\&P500 수익률(`ret_sp500_bp`)과 코스피200 수익률(`ret_kospi200_bp`, `ret_kospi200_lead_bp`)의 상관관계는?

```{r, echo=FALSE}
# Stooq CSV 다운로드 시도 후 결합하는 함수
fetch_stooq <- function(symbol_candidates, value_name) {
  for (sym in symbol_candidates) {
    ...
    dat <- tryCatch(read.csv(url), error = function(e) NULL)
    if (!is.null(dat) && all(c("Date", "Close") %in% names(dat)) && nrow(dat) > 0) {
      out <- as_tibble(dat) %>%
        transmute(date = as.Date(Date), !!value_name := as.numeric(Close)) %>%
        arrange(date)
      if (nrow(out) > 0 && sum(is.finite(out[[value_name]])) > 100) return(out)
    }
  }
  NULL
}

sp500_px <- fetch_stooq(c("^spx", "^gspc", "spx.us"), "sp500_close")
kospi200_px <- fetch_stooq(c("^ks200", "^kospi200", "ks200.kr"), "kospi200_close")

# 외부 접속이 안 되는 환경에서는 렌더가 끊기지 않도록 대체 샘플 사용
if (is.null(sp500_px) || is.null(kospi200_px)) {
  fallback <- as_tibble(as.data.frame(EuStockMarkets)) %>%
    transmute(
      date = seq.Date(as.Date("1991-01-01"), by = "day", length.out = nrow(EuStockMarkets)),
      # 대체 샘플에서는 FTSE/DAX를 각각 S&P500/KOSPI200의 프록시로 사용
      sp500_close = FTSE,
      kospi200_close = DAX
    )
  merged_px <- fallback
} else {
  merged_px <- inner_join(sp500_px, kospi200_px, by = "date")
}

returns_raw <- merged_px %>%
  mutate(
    ret_sp500_bp = 10000 * (log(sp500_close) - log(lag(sp500_close))),
    ret_kospi200_bp = 10000 * (log(kospi200_close) - log(lag(kospi200_close)))
  ) %>%
  mutate(ret_kospi200_lead_bp = lead(ret_kospi200_bp)) %>%
  drop_na() %>%
  mutate(ret_sp500_bin = round(ret_sp500_bp / 10) * 10)
```
...
g_math = ggplot(returns_raw, aes(x = ret_sp500_bp, y = ret_kospi200_bp)) + 
...
::: {.column width="50%"}
```{r, echo=FALSE,fig.height=6}
g_verb = ggplot(returns_raw, aes(x = ret_sp500_bp, y = ret_kospi200_lead_bp)) + 
    geom_point(size = 2, alpha = 0.5) +
    xlim(-400, 400) +
    ylim(-450, 450) +
    labs(x = "S&P500 return (bp)",
         y = "Return (bp)",
...
-   상관계수에서 보듯이 양(+)의 선형 관계가 보임. S&P500 수익률을 구간(bin)으로 묶어 평균 관계를 더 명확하게 살펴보자.
...
returns_bin <- returns_raw %>%
  ...
  summarise(ret_kospi200_bin = mean(ret_kospi200_bp),
            ret_kospi200_lead_bin = mean(ret_kospi200_lead_bp),
            .groups = "drop")

g_math_cs = ggplot(returns_bin, aes(x = ret_sp500_bin, y = ret_kospi200_bin)) + 
    geom_point(size = 2) +
    xlim(-400, 400) +
    ylim(-450, 450) +
    labs(
      x = "S&P500 return bin (bp)",
      y = "Average return (bp)",
      title = "KOSPI200 (same day)") +
    theme_bw(base_size = 20)
g_math_cs
...
::: {.column width="50%"}
```{r, echo=FALSE,fig.height=6}
g_verb_cs = ggplot(returns_bin, aes(x = ret_sp500_bin, y = ret_kospi200_lead_bin)) + 
    geom_point(size = 2) +
    xlim(-400, 400) +
    ylim(-450, 450) +
    labs(
...
-   두 변수 간의 관계에 관심 있음:
    ...
    -   결과변수 (종속변수, dependent variable): *KOSPI200 return (bp)* $(y)$

    -   설명변수 (독립변수, independent variable): *S&P500 return (bp)* $(x)$
...
\begin{aligned}
\widehat y &= b_0 + b_1 x \\
\widehat {\text{KOSPI200 return (bp)}} &= b_0 + b_1 \cdot \text{S\&P500 return (bp)}
\end{aligned}
$$
...
$$
\begin{aligned}
\widehat {\text{KOSPI200 return (bp)}} &= b_0 + b_1 \cdot 30
\end{aligned}
$$
